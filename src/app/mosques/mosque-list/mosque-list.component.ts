...
export class MosqueListComponent implements OnInit, AfterViewInit {
  ...
  getMosqueList() {
    this.spinner.show();
    this.mosquesService.getMosqueList({}).subscribe(
      (res: any) => {
        this.spinner.hide();
        if (res?.status) {
          this.mosqueList = res?.mosqueList || [];
  ...
  toggleActive(e: any) {
    const text = e.is_active
      ? 'You want to deactivate this mosque !'
      : 'You want to Activate this mosque !';
    const confirmButtonText = e.is_active ? 'Yes deactivate it !' : 'Yes Activate it !';
    Swal.fire({
      title: 'Are you sure?',
      text: text,
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        this.spinner.show();
        const data = { _id: e._id, is_active: !e.is_active };
        this.mosquesService.activeDeactiveMosque(data).subscribe(
          (res: any) => {
            this.spinner.hide();
            if (res.status) {
          ...
          (err: any) => {
            this.spinner.hide();
            if (err?.error?.message == 'jwt expired') this.router.navigateByUrl('/login');
            console.error(err);
          }
        );
      }
  ...
  deleteMosque(e: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.spinner.show();
        this.mosquesService.deleteMosque({ _id: e._id }).subscribe(
          (res: any) => {
            this.spinner.hide();
            if (res.status) {
              this.toastr.success(res.message, 'Success');
